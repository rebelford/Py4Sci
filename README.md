...
Please Note: The first Lesson is a Jupyter Notebook on Installing Jupyter Lab. You can not run it without having Jupyter installed, so I create an HTML Version [here](https://rebelford.github.io/IOCT-TECH/01_aSetUpComputer%20.html)
...
This GitHub Repository has several directories and here is an outline of what is in each
...
4. **Module3-Packages** - This module will go over several packages.  It will start with an exploratoin of the OOP (Object Oreinted Programing) structures the package provides (functions, class objects and their methods) and then provide some activities demonstrating the application of those objects to various scientific tasks.
5. **My_Package** - This is an activity we will perform where we make our own low-level package that has functions we can import into other notebooks.
6. **Mordred_rdkit_env** - This is an activity from the [2019 Cheminformatics OLCC](https://chem.libretexts.org/Courses/Intercollegiate_Courses/Cheminformatics/05%3A_5._Quantitative_Structure_Property_Relationships) that has deprecated package dependencies and so you need to install versions of Python and RdKit that work with Mordred from 2019. The goal here is to show that you can create an environment using old deprecated packages that will work, and not interfer with other environments that have current (updated) packages.
7. **ai_books** -  You are expected to create a directory of jupyter notebooks that record your discussions with AIs.  You are to copy the AI output to a markdown cells and to experiment with code in code cells.  This is sort of your lab notebook where you experiment with various things you try to develop with the assistance of an AI.  Each book should have a name like ai_conda, ai_matplotlib, ai_dictionaries, where the "ai_" indicates this is your dialgoue with an AI, and what follows is the topic of that dialogue
8. **Search_Utility** - This directory contains a search utility program you can download to your computer and search all the Jupyter notebooks in your home directory, or a specified subdirectory of your home directory. This allows you to quickly navigate all assignments and ai_books
...
- **search_results/** - Automagically created the first time you run the search-utility program.  You should move that program to this folder after it is created. Every time you run a search an html file will be generated with a name like search_search-term.html and placed in this directory. So if I searched for csv it would be called search_csv.html.  When you open this in the Jupyter Lab an html page will open with links to different notebooks, followed by a list of all headers that contain that link.  You can not open the notebook in the current notebook, but have to open in a new tab or new window.  What I do is open multiple notebooks in different tabs, and then browse through them to find what I need, allowing me to quickly look through all my notebooks that have a topic I am working on. This should not be a git repo as you are searching your own work space.
- **ai_books/** - When I engaged an AI on a search topic I copy the results into a markdown cell of a notebook named ai_searchtopic.ipynb. So if I am collecting information on csv, the notebook would be called ai_csv.ipynb.  When you use the copy option in ChatGPT or Perplexity.ai it is formatted in Markdown and generates headers that can be found by the search utility.  I will often design new headers to allow me to find topics by search terms, and I will also create small code cells that demonstrate coding concepts. I would not make this a git repo, as this is my **AI-Assisted Learning Journal**, a collection of my interactions with AIs targeting a specific topic.
